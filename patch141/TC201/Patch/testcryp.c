/*  ----------------------------------------------------------------<Prolog>-
    ...
    Written:    96/05/22  Pieter Hintjens <ph@imatix.com>
...
test_crypt (FILE *file, int crypt_type)
{
    ...
    clk_begin = clock ();
    while ((read_length = fread (read_buffer, 1, 16000, file)) != 0) /* Change 32000 to 16000 for Turbo C 2.01 -ms memory model */
      {
        nb_block = (long)(read_length / 16);		/* Change 32 to 16 for Turbo C 2.01 -ms memory model */
        if (nb_block * 16 != (long)read_length)		/* Change 32 to 16 for Turbo C 2.01 -ms memory model */
            read_length = (size_t)(nb_block * 16);	/* Change 32 to 16 for Turbo C 2.01 -ms memory model */
        memcpy (test_buffer, read_buffer, 16000);	/* Change 32000 to 16000 for Turbo C 2.01 -ms memory model */
